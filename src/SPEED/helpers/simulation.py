...
class SimulationHelper(Helper):
    ...
    def convert_environment_using_topology_zoo(zoo_topology: str, zone_root: int, environment_file: str, topo_file: str):
        ...
        1 - Create the aggregation zone for each node in the zoo topology
        2 - Assing each compute zone for the aggregation zone
        3 - Create the links between the
        4 - Create the Hierarchy topology based on the zoo_root.
            4.1 - Remove all the links between nodes in differente domains
            4.2 - For each link in the zoo topology bind two nodes for the compute node of the zone
        ...
        #     for node in environment['domains'][domain]['nodes']:
        #         aux_nodes[node] = domain
        #
        # links_copy = environment['links'].copy()
        # for link in environment['links']:
        #     dom_src = aux_nodes[environment['links'][link]['src']]
        #     dom_dst = aux_nodes[environment['links'][link]['dst']]
        #     if dom_src != dom_dst:
        #         del(links_copy[link])
        #
        # new_links = {}
        # for edge in tree.edges:
        #     dom_src = "dom_{}".format(edge[0])
        #     dom_dst = "dom_{}".format(edge[1])
        #
        #     node_src = random.choice(environment['domains'][dom_src]['nodes'])
        #     node_dst = random.choice(environment['domains'][dom_dst]['nodes'])
        #
        #     aux_link_selected = random.choice(list(environment['links'].keys()))
        #     link_selected = environment['links'][aux_link_selected]
        #     link_selected['src'] = node_src
        #     link_selected['dst'] = node_dst
        #     link_selected['bw'] = 1000000
        #
        #     nane_new_link = "l_{}_{}".format(node_src, node_dst)
        #     new_links[nane_new_link] = link_selected
        #
        # l = links_copy.copy()
        # l.update(new_links)
        # environment['links'] = l
        #
        # TopologyGeneratorHelper.save_file(
        #     output_file=format(environment_file),
        #     data=environment
        # )
